...
exports.createFourController = createFourController;
const renderer_1 = require("../rendering/renderer");
const gameState_1 = require("../state/gameState");
function createFourController(W_init, H_init, WIN_SCORE, paddleSpeed) {
    let p4ctx = null;
    ...
    function init(names) {
        const cv = document.getElementById('game4');
    ...
    function step() {
        ...
        p4Ball.x += p4Ball.vx;
        p4Ball.y += p4Ball.vy;
        let scoredIndex = null;
        p4Players.forEach((p, idx) => {
            if (p.dir === 'V') {
                const inY = p4Ball.y > p.y && p4Ball.y < p.y + p.h;
                if (idx === 0 && p4Ball.x - p4Ball.r < p.x + p.w && inY) {
                    p4Ball.vx = Math.abs(p4Ball.vx);
                }
                if (idx === 1 && p4Ball.x + p4Ball.r > p.x && inY) {
                    p4Ball.vx = -Math.abs(p4Ball.vx);
                }
                if (idx === 0 && p4Ball.x < 0)
                    scoredIndex = 1;
                if (idx === 1 && p4Ball.x > p4W)
                    scoredIndex = 0;
            }
            else {
                const inX = p4Ball.x > p.x && p4Ball.x < p.x + p.w;
                if (idx === 2 && p4Ball.y - p4Ball.r < p.y + p.h && inX) {
                    p4Ball.vy = Math.abs(p4Ball.vy);
                }
                if (idx === 3 && p4Ball.y + p4Ball.r > p.y && inX) {
                    p4Ball.vy = -Math.abs(p4Ball.vy);
                }
                if (idx === 2 && p4Ball.y < 0)
                    scoredIndex = 3;
                if (idx === 3 && p4Ball.y > p4H)
                    scoredIndex = 2;
            }
        });
        if (scoredIndex != null) {
            ...
            if (!p4Winner && p4Players[scoredIndex].score >= WIN_SCORE) {
                ...
                try {
                    ...
                    p4Players.forEach((pl, idx) => {
                        ...
                        (0, gameState_1.recordMatch)(w, l, w, wScore, lScore);
                    });
                }
                catch { }
            }
        ...
        (0, renderer_1.drawFour)(p4ctx, { p4W, p4H, p4Players, p4Ball, p4Winner, lang: document.documentElement.lang || 'en' });
    }
    function startIfReady(playersList) {
    ...
    function handleKey(e) {
        ...
        switch (e.code) {
            case 'KeyW':
                p4Players[0].y -= P4_SPEED;
                break;
            case 'KeyS':
                p4Players[0].y += P4_SPEED;
                break;
            case 'ArrowUp':
                p4Players[1].y -= P4_SPEED;
                break;
            case 'ArrowDown':
                p4Players[1].y += P4_SPEED;
                break;
            case 'KeyA':
                p4Players[2].x -= P4_SPEED;
                break;
            case 'KeyD':
                p4Players[2].x += P4_SPEED;
                break;
            case 'KeyJ':
                p4Players[3].x -= P4_SPEED;
                break;
            case 'KeyL':
                p4Players[3].x += P4_SPEED;
                break;
        ...
        if (p4Players[0])
            p4Players[0].y = Math.max(8, Math.min(p4H - p4Players[0].h - 8, p4Players[0].y));
        if (p4Players[1])
            p4Players[1].y = Math.max(8, Math.min(p4H - p4Players[1].h - 8, p4Players[1].y));
        if (p4Players[2])
            p4Players[2].x = Math.max(8, Math.min(p4W - p4Players[2].w - 8, p4Players[2].x));
        if (p4Players[3])
            p4Players[3].x = Math.max(8, Math.min(p4W - p4Players[3].w - 8, p4Players[3].x));
    }
    return { init, step, startIfReady, handleKey };
}
//# sourceMappingURL=fourPlayer.js.map